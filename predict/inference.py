...
from tplinker_plus import (DataMaker4Bert,
                           DataMaker4BiLSTM,
                           TPLinkerPlusBert,
                           TPLinkerPlusBiLSTM,
                           MetricsCalculator)
...
for file_path in glob.glob(test_data_path):
    file_name = re.search("(.*?)\.json", os.path.basename(file_path)).group(1)
...
if config["hyper_parameters"]["force_split"] or experiment_name.startswith("image_report"):
    split_test_data = True
...
if split_test_data:
    ...
    for file_name, data in ori_test_data_dict.items():
        if experiment_name.startswith("image_report"):
            test_data_dict[file_name] = preprocessor.split_into_short_samples_re(
                data,
                max_length=config["hyper_parameters"]['max_seq_len'])
        else:
...
if config["encoder"] == "BERT":
    ...
    if config["pretrained_format"] == "glove":
        ...
        wv = KeyedVectors.load_word2vec_format(config["pretrained_word_embedding_path"], binary=False)
        dictionary = wv.key_to_index
        word_vectors = wv.vectors

    # prepare embedding matrix
    word_embedding_init_matrix = np.random.normal(
        -1, 1, size=(len(token2idx), config['hyper_parameters']["word_embedding_dim"])
    )
    count_in = 0
    ...
    for ind, tok in tqdm(idx2token.items(), desc="Embedding matrix initializing..."):
        if tok in dictionary:
            count_in += 1
            word_embedding_init_matrix[ind] = word_vectors[dictionary[tok]]
    ...
    rel_extractor = TPLinkerPlusBiLSTM(word_embedding_init_matrix,
                                       config["hyper_parameters"]["emb_dropout"],
                                       config["hyper_parameters"]["enc_hidden_size"],
                                       config["hyper_parameters"]["dec_hidden_size"],
                                       config["hyper_parameters"]["rnn_dropout"],
                                       tag_size,
                                       config["hyper_parameters"]["shaking_type"],
                                       config["hyper_parameters"]["inner_enc_type"],
                                       )
...
def get_last_k_paths(path_list, k):
    path_list = sorted(path_list, key=lambda x: int(re.search("(\d+)", os.path.basename(x)).group(1)))
    #     pprint(path_list)
    return path_list[-k:]
...
def filter_duplicates(rel_list, ent_list):
    ...
    for rel in rel_list:
        rel_memory = "{}\u2E80{}\u2E80{}\u2E80{}\u2E80{}".format(rel["subj_char_span"][0],
                                                                 rel["subj_char_span"][1],
                                                                 rel["predicate"],
                                                                 rel["obj_char_span"][0],
                                                                 rel["obj_char_span"][1])
    ...
    filtered_ent_list = []
    for ent in ent_list:
        ent_memory = "{}\u2E80{}\u2E80{}".format(ent["char_span"][0],
                                                 ent["char_span"][1],
                                                 ent["type"])
    ...
    for i, batch_test_data in enumerate(tqdm(test_dataloader, desc="Predicting")):
        ...
        for ind in range(len(sample_list)):
            ...
            for item in ent_list:
                if item['conf'] < config['activate_learning_thresh']:
...
def get_test_prf(pred_sample_list, gold_test_data, pattern="whole_text"):
    ...
    for sample in gold_test_data:
        text_id = sample["id"]
        relation_list, entity_list = filter_duplicates(sample.get("relation_list", []), sample.get("entity_list", []))
...
for file_name, short_data in test_data_dict.items():
    ...
    for run_id, model_path_list in run_id2model_state_paths.items():
        ...
        for model_state_path in model_path_list:
            res_num = re.search("(\d+)", os.path.basename(model_state_path)).group(1)
            ...
            if os.path.exists(save_path):
                ...
                # load model state
                rel_extractor.load_state_dict(torch.load(model_state_path, map_location=device))
                rel_extractor.eval()
                print("run_id: {}, model state {} loaded".format(run_id, os.path.basename(model_state_path)))

                # predict
                pred_sample_list, df_al = predict(short_data, ori_test_data)

            res_dict[save_path] = (pred_sample_list, df_al)
            predict_statistics[save_path] = len([s for s in pred_sample_list if len(s["relation_list"]) > 0])

pprint(predict_statistics)

# check
for path, (pred_sample_list, df_al) in res_dict.items():
    for sample in tqdm(pred_sample_list, desc="check char span"):
        text = sample["text"]
        for rel in sample["relation_list"]:
...
if config["save_res"]:
    for path, (pred_sample_list, df_al) in res_dict.items():
        with open(path, "w", encoding="utf-8") as file_out:
            for sample in tqdm(pred_sample_list, desc="Output"):
...
if config["score"]:
    filepath2scores = {}
    for file_path, (pred_sample_list, df_al) in res_dict.items():
        file_name = re.search("(.*?)_res_\d+\.json", os.path.basename(file_path)).group(1)
        gold_test_data = ori_test_data_dict[file_name]
        prf_dict, df_score = get_test_prf(pred_sample_list, gold_test_data,
