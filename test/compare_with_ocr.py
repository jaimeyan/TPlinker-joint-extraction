...
class Predictor():
    ...
    def predict(self, test_data):
        ...
        test_dataloader = DataLoader(MyDataset(indexed_test_data),
                                     ...
                                     num_workers=0,
                                     drop_last=False,
                                     collate_fn=lambda data_batch: self.data_maker.generate_batch(data_batch,
                                                                                                  data_type="test"),
                                     )
    ...
    def get_test_prf(self, pred_sample_list, gold_test_data, pattern="whole_text", if_only_lis=False):
        ...
        for gold_n_pred in text_id2gold_n_pred.values():
            ...
            if pattern == "event_extraction":
                pred_event_list = self.handshaking_tagger.trans2ee(pred_rel_list,
                                                                   pred_ent_list)  # transform to event list
                gold_event_list = gold_n_pred["gold_event_list"]  # *
                self.metrics.cal_event_cpg(pred_event_list, gold_event_list, ee_cpg_dict)
            else:
                self.metrics.cal_rel_cpg(pred_rel_list, pred_ent_list, gold_rel_list, gold_ent_list, ere_cpg_dict,
                                         pattern)
        ...
        if pattern == "event_extraction":
            ...
            arg_class_prf = self.metrics.get_prf_scores(ee_cpg_dict["arg_class_cpg"][0],
                                                        ee_cpg_dict["arg_class_cpg"][1],
...
def lis_item_word_parse(lis_info):
    lis_item_list, lis_relation_dict = [], {}
    for lis in lis_info:
        lis_item_list.append(lis['name'])
        lis_relation_dict[lis['name']] = {'代号': lis['abbr'],
                                          '结果值': lis['value'],
                                          '单位': lis['unit'],
                                          '参考': lis['refer']}

    return lis_item_list, lis_relation_dict
...
def fetch_lis_items_from_pic(image_path, data_json):
    ...
    files = {"img": open(image_path, 'rb'),
             'data': open(data_json, 'rb')}
    headers = {}
    response = requests.request("POST", ocr_url, headers=headers, files=files)
    if response.status_code != 200:
        print("request ocr error!")
    json_data = json.loads(response.content)
    lis_item_list, list_relation_list = [], []
    basic_info = {'姓名': None, '年龄': None, '床号': None, '性别': None, '科室': None, '收样时间': None, '报告时间': None,
                  '检验时间': None}
    if 'examinationList' in json_data['data']['img_data_list'][0]['extract_info']:
        if json_data['data']['img_data_list'][0]['extract_info']['basicInfos']:
            for i in json_data['data']['img_data_list'][0]['extract_info']['basicInfos']:
                if i['key'] in basic_info:
                    basic_info[i['key']] = i['content']
                if i['key'] == '送检科室':
                    basic_info['科室'] = i['content']
        lis_info = json_data['data']['img_data_list'][0]['extract_info']['examinationList']
        lis_item_list, list_relation_list = lis_item_word_parse(lis_info)
    if json_data['code'] == 200:
        pass

    return lis_item_list, list_relation_list, basic_info


def get_ocr_emr_res(pic_names):
    pics_dict = {}
    for parent, dirnames, filenames in os.walk('test_pics'):
        for filename in filenames:
            if filename in pic_names:
                pics_dict[filename] = os.path.join(parent, filename)

    res_dict = {}
    for pic in pics_dict:
        res_dict[pic] = fetch_lis_items_from_pic(pics_dict[pic], data_json='ocr_request.json')
    return res_dict


def get_re_ent_from_model(sample):
    lis_item_list, list_relation_list, basic_info_dict = [], [], {
        '姓名': None, '年龄': None, '床号': None, '性别': None, '科室': None, '收样时间': None, '报告时间': None, '检验时间': None}

    for ent in sample['entity_list']:
        if ent['type'] == '检验项目':
            lis_item_list.append(ent['text'])
        elif ent['type'] in basic_info_dict:
            basic_info_dict[ent['type']] = ent['text']
    for re in sample['relation_list']:
        if re['predicate'] == '收样时间标签-收样时间':
            basic_info_dict['收样时间'] = re['subject']
        elif re['predicate'] == '报告时间标签-报告时间':
            basic_info_dict['报告时间'] = re['subject']
        elif re['predicate'] == '检验时间标签-检验时间':
            basic_info_dict['检验时间'] = re['subject']

    re_dict = {}
    for lis_item in lis_item_list:
        re_dict[lis_item] = {'代号': None,
                             '结果值': None,
                             '单位': None,
                             '参考': None}
        for re in sample['relation_list']:
            if re['object'] == lis_item:
                if re['predicate'] == '检验项目-结果':
                    re_dict[lis_item]['结果值'] = re['subject']
                elif re['predicate'] == '检验项目-参考范围':
                    re_dict[lis_item]['参考'] = re['subject']
                elif re['predicate'] == '检验项目-单位':
                    re_dict[lis_item]['单位'] = re['subject']
                elif re['predicate'] == '检验项目-代号':
                    re_dict[lis_item]['代号'] = re['subject']

    return lis_item_list, re_dict, basic_info_dict


def get_ent_rel_cpg(pred_sample, gold_sample, rel_cpg, ent_cpg, rel_cpg_all, ent_cpg_all):
    pred_lis_list, pred_re_dict, pred_basic_dict = pred_sample
    gold_lis_list, gold_re_dict, gold_basic_dict = gold_sample

    ent_cpg['检验项目'][2] += len(gold_lis_list)
    ent_cpg['检验项目'][1] += len(pred_lis_list)
    for i in gold_lis_list:
        ent_cpg_all[2] += 1
    for i in pred_lis_list:
        ent_cpg_all[1] += 1
        if i in gold_lis_list:
            ent_cpg['检验项目'][0] += 1
            ent_cpg_all[0] += 1

    for i in gold_basic_dict:
        if gold_basic_dict[i]:
            ent_cpg[i][2] += 1
            ent_cpg_all[2] += 1
    for i in pred_basic_dict:
        if pred_basic_dict[i]:
            ent_cpg[i][1] += 1
            ent_cpg_all[1] += 1
            if pred_basic_dict[i] == gold_basic_dict[i]:
                ent_cpg[i][0] += 1
                ent_cpg_all[0] += 1

    for re in gold_re_dict:
        for attr in gold_re_dict[re]:
            if gold_re_dict[re][attr]:
                rel_cpg['项目名称-{}'.format(attr)][2] += 1
                rel_cpg_all[2] += 1

    for re in pred_re_dict:
        for attr in pred_re_dict[re]:
            if pred_re_dict[re][attr]:
                rel_cpg['项目名称-{}'.format(attr)][1] += 1
                rel_cpg_all[1] += 1
                if re in gold_re_dict and pred_re_dict[re][attr] == gold_re_dict[re][attr]:
                    rel_cpg['项目名称-{}'.format(attr)][0] += 1
                    rel_cpg_all[0] += 1


def get_prf(MyPredictor, pred_samples, gold_samples):
    rel_cpg_all = [0, 0, 0]
    ent_cpg_all = [0, 0, 0]
    rel_cpg = {rel: [0, 0, 0] for rel in {'项目名称-代号', '项目名称-结果值', '项目名称-单位', '项目名称-参考'}}
    ent_cpg = {ent: [0, 0, 0] for ent in {'检验项目', '姓名', '年龄', '床号', '性别', '科室', '收样时间', '报告时间', '检验时间'}}

    for gold_sample_key in gold_samples:
        gold_sample = gold_samples[gold_sample_key]
        if gold_sample_key in pred_samples:
            pred_sample = pred_samples[gold_sample_key]
            get_ent_rel_cpg(pred_sample, gold_sample, rel_cpg, ent_cpg, rel_cpg_all, ent_cpg_all)

    metrics = MetricsCalculator(MyPredictor.handshaking_tagger)
    # handshaking_tagger = HandshakingTaggingScheme(Predictor.rel2id, Predictor.max_seq_len, Predictor.ent2id)
    rel_prf = metrics.get_prf_scores(rel_cpg_all[0], rel_cpg_all[1],
                                     rel_cpg_all[2])
    ent_prf = metrics.get_prf_scores(ent_cpg_all[0], ent_cpg_all[1],
                                     ent_cpg_all[2])
    prf_dict = {
        "rel_prf": rel_prf,
        "ent_prf": ent_prf,
    ...
    df_columns = ["correct", "predict", "gold", "precision", "recall", "f1"]
    df_indices = ["rel", "ent"]
    df_data = [(*rel_cpg_all, *rel_prf), (*ent_cpg_all, *ent_prf)]
    for k, v in rel_cpg.items():
        prf_dict["rel_" + k + "_prf"] = metrics.get_prf_scores(*v)
        df_indices.append("rel_" + k)
        df_data.append((*v, *prf_dict["rel_" + k + "_prf"]))
    for k, v in ent_cpg.items():
        prf_dict["ent_" + k + "_prf"] = metrics.get_prf_scores(*v)
        df_indices.append("ent_" + k)
        df_data.append((*v, *prf_dict["ent_" + k + "_prf"]))
...
if __name__ == '__main__':
    ...
    pic_names = []
    pred_samples, gold_samples = {}, {}
    gold_sample_list = test_data_dict['huayandan_test_0620']
    for i in gold_sample_list:
        pic_names.append(i['id'])
        gold_samples[i['id']] = get_re_ent_from_model(i)
    for i in pred_sample_list:
        pred_samples[i['id']] = get_re_ent_from_model(i)

    ocr_samples = get_ocr_emr_res(pic_names)

    prf_dict_pred, df_pred = get_prf(MyPredictor, pred_samples, gold_samples)
    prf_dict_ocr, df_ocr = get_prf(MyPredictor, ocr_samples, gold_samples)

    df_pred.to_excel('../results/huayandan4/ocr_compare/huayandan_test_0620_res_model_19_metric.xlsx')
    df_ocr.to_excel('../results/huayandan4/ocr_compare/huayandan_test_0620_res_ocr_metric.xlsx')
    #
